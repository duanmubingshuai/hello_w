...
uint8_t SimpleGATTProfile_Notify(uint16 connHandle,uint8 len, void* value )
{
	#ifdef _PHY_DEBUG 
		LOG("%s,%s,Line %d\n",__FILE__,__func__,__LINE__);
	...
	if ( simpleGATTProfileChar2Config & GATT_CLIENT_CFG_NOTIFY )
    {
        ...
        return(GATT_Notification( connHandle, &Notif, FALSE ));
