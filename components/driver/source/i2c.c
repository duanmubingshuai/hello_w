...
int hal_i2c_pin_init(i2c_dev_t dev, gpio_pin_e pin_sda, gpio_pin_e pin_clk){
	if(dev == I2C_0)
	{
		gpio_fmux_set(pin_clk, FMUX_IIC0_SCL);
		gpio_fmux_set(pin_sda, FMUX_IIC0_SDA);
	}
//  else if(dev == I2C_1){
//		gpio_fmux_set(pin_clk, FMUX_IIC1_SCL);
//		gpio_fmux_set(pin_sda, FMUX_IIC1_SDA);
//  }
	else
	{
		return PPlus_ERR_INVALID_PARAM;
	}
	gpio_pull_set(pin_sda,STRONG_PULL_UP);
	gpio_pull_set(pin_clk,STRONG_PULL_UP);
...
int hal_i2c_read_s(void* pi2c, uint8_t slave_addr, uint8_t reg, uint8_t* data, uint8_t size)
{
	I2C_INIT_TOUT(to);
	AP_I2C_TypeDef * pi2cdev = (AP_I2C_TypeDef *)pi2c;
