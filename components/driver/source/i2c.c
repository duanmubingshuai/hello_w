...
{P18,P15, FMUX_P18_IIC_SCL,FMUX_P15_IIC_SDA},
};
typedef enum{
	hcnt_mod = 0,
	lcnt_mod = 1,
...
void master_send_read_cmd(uint8_t len)
{
	...
	for(i=0; i<len; i++)
	{
		WaitUs(50);                     //The transmitter response speed is not fast enough, so need to wait for 1us
...
int i2c_read_s(uint8_t slave_addr, uint8_t reg, uint8_t* data, uint8_t size)
{	
  ...
  )
{
	...
	while(size)
	{
		...
		ret = i2c_read_s(slave_addr, reg, data, cnt);
...
//	AP_I2C_TypeDef * pi2cdev = (AP_I2C_TypeDef *)AP_I2C0;
	...
	while(size)
	{
		...
		i2c_addr_update(slave_addr|(reg>>8));
		...
		if(PPlus_ERR_TIMEOUT == I2C_CHECK_TOUT(i2c_op_timeout*size, "I2C WR TO\n"))
				return PPlus_ERR_TIMEOUT;
