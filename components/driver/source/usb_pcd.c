...
uint32_t usb_ep_tx_push(uint8_t qid, uint8_t *buf, uint32_t len)
{
    ...
    else
    {
        ...
        for(i = usb_ep_tx_q[qid].num; i>0 ; i--){
            osal_memcpy(&(usb_ep_tx_q[qid].pkg[i].bdata[0]),
                &(usb_ep_tx_q[qid].pkg[i-1].bdata[0]), sizeof(usb_hid_pkg_t));
        }
        osal_memcpy(&(usb_ep_tx_q[qid].pkg[0].bdata[0]), buf, len);
        usb_ep_tx_q[qid].num++;
		usb_ep_tx_q[qid].pkg[0].size_len=len;
    }
...
void usb_hid_tx_send(usb_hal_pcd_t *hpcd)
{
    ...
    if(hid_mouse_ready && hid_remote_wakeup_rdy == HID_REMOTE_WAKEUP_IDLE)
    {
        ...
        for(i = 1; i< MAX_EP_NUM; i++){
            ...
            if(pkg){
                ...
                hpcd->in_ep_regs[ep_num]->in.EP_IN_TRANS_SZ = pkg->size_len;
                ...
                //gpio_write(P14, 0);
				uint8 k=0;
				for(k=0;k<pkg->size_len/4;k++)
				{
					write_reg(fifo,  data[k]);
				}
				if(pkg->size_len%4)
				{
					write_reg(fifo,  data[k]);
...
//            LOG_DEBUG("tx_xfer_cnt = %d, xfer_tmpcnt=%d\n", hpcd->ep[ep_num].xfer_cnt, hpcd->ep[ep_num].xfer_tmpcnt);
    ...
    if (tx_sts.b.NAK_SENT)
    {
        ...
        if (hpcd->ep[ep_num].xfer_buf != NULL)
        {
            ...
            {
            #if _USB_LS_SPEED
				if (hpcd->ep[ep_num].xfer_length % 8 == 0)
			#else	 
...
static void usb_pcd_svc_rx_endp(usb_hal_pcd_t *hpcd, uint16_t ep_num)
{
    ...
    if (out_fifo_sts.b.STATUS_COMP)
    {
        if (out_fifo_sts.b.STATUS)
        {
			...
			if(out_fifo_sts.b.TRANSFER_SZ && _usb_out_cb_f)
				_usb_out_cb_f((uint32 *)hpcd->rx_fifo[ep_num], rx_size);
...
static void usb_pcd_setup_proc(usb_hal_pcd_t *hpcd, ureq_t req)
{
    ...
    else
    {
		...
		#if _USB_LS_SPEED
            hal_pcd_ep_info_set(hpcd, 0, HAL_PCD_EP_DIR_OUT, HAL_PCD_EP_TYPE_CTRL, 8);
		#else
...
inline void add_ep_info(usb_hal_pcd_t *hpcd0 ,uint8 ep_id,hal_ep_type_t ep_type,uint16 max_pkt,uint8 fifo_deep)
{
	dev_ep_info_t ep_info;
	dev_sts_t dev_sts;
	dev_sts.d32 = hpcd0->dev_glb_regs->DEV_STS;
	ep_info.d32 = 0;
	ep_info.d32 = hpcd0->mac_ep_regs->DEV_EP_INFO[ep_id];
	ep_info.b.EP_DIR = HAL_PCD_EP_DIR_IN;
	ep_info.b.EP_NUM = ep_id;
	ep_info.b.CFG_NUM = dev_sts.b.CFG;
	ep_info.b.EP_TYPE = ep_type;
	ep_info.b.MAX_PKT = max_pkt;
	hpcd0->mac_ep_regs->DEV_EP_INFO[ep_id] = ep_info.d32;
	hpcd0->in_ep_regs[ep_id]->in.EP_IN_FIFO_SZ = fifo_deep;

	hpcd0->ep[ep_id].xfer_buf = 0;
	hpcd0->ep[ep_id].xfer_cnt = 0;
	hpcd0->ep[ep_id].xfer_tmpcnt = 0;
	hpcd0->ep[ep_id].xfer_length = 0;
	hpcd0->ep[ep_id].max_packet = max_pkt;
	hpcd0->ep[ep_id].fifo_depth = fifo_deep;
	if(ep_type==HAL_PCD_EP_TYPE_INTR)
	{
		hpcd0->ep[ep_id].is_in  = 1;
	}
	else if(ep_type==HAL_PCD_EP_TYPE_ISOC)
	{
		hpcd0->ep[ep_id].is_in  = 1;
		hpcd0->ep[ep_id].is_iso  = 1;
	}
}
inline void flush_ep_fifo(usb_hal_pcd_t *hpcd0 ,uint8 ep_id)
{
	ep_in_ctl_t ep_in_ctl;
	ep_in_ctl.d32 = 0;
	ep_in_ctl.d32 = hpcd0->in_ep_regs[ep_id]->in.EP_IN_CTL;
	ep_in_ctl.b.FIFO_READY = 0;
	ep_in_ctl.b.FLUSH_FIFO = 1;
	hpcd0->in_ep_regs[ep_id]->in.EP_IN_CTL = ep_in_ctl.d32;
}

#if ( _DEF_USB_INFO_ == USB_UAC_CONSUMER_KBD )

bool uac_open_data_stream;
extern uint8 application_TaskID;
#define ISOC_TRANSFER_EVT				0x0200	

void update_iso_fifo_flush(usb_hal_pcd_t *hpcd,uint32 *data,uint8 len)
{
	ep_in_ctl_t tx_ctl;
	dev_sts_t dev_sts;
	uint32 now_frame_num = 0;
	dev_sts.d32=hpcd->dev_glb_regs->DEV_STS;
	now_frame_num = dev_sts.b.TS;
	if (hpcd->ep[ISO_EP].is_iso)
	{
		volatile uint32_t *even_frame_fifo=hpcd->tx_fifo[ISO_EP];
		/*1. Write frame_numbor   		*/
		hpcd->dev_glb_regs->FRAME_NUM = now_frame_num;
		/*2. Write trnasfer Size   		*/
		hpcd->in_ep_regs[ISO_EP]->in.EP_IN_TRANS_SZ =len*4;  //uint32 ==  4bytes
		/*3. Write trnasfer data fifo   */
		for(uint8 i=0;i<len;i++)
		{
			write_reg(even_frame_fifo,  data[i]);
		}
		/*4. Set FIFO Ready        */
		tx_ctl.d32 = hpcd->in_ep_regs[ISO_EP]->in.EP_IN_CTL;
		tx_ctl.b.FIFO_READY = 1;
		hpcd->in_ep_regs[ISO_EP]->in.EP_IN_CTL = tx_ctl.d32;
	}
}
#endif
...
#if ( _DEF_USB_INFO_ == USB_UAC_CONSUMER_KBD )
		add_ep_info(hpcd,ISO_EP,HAL_PCD_EP_TYPE_ISOC,64,16);
        add_ep_info(hpcd,CONTROL_EP,HAL_PCD_EP_TYPE_INTR,64,16);
		#ifdef ADD_KEYBOARD
		add_ep_info(hpcd,KEYBOARD_EP,HAL_PCD_EP_TYPE_INTR,64,16);
		#endif
#else
		#if _USB_LS_SPEED
		add_ep_info(hpcd,1,HAL_PCD_EP_TYPE_INTR,8,16);
		#else
		add_ep_info(hpcd,1,HAL_PCD_EP_TYPE_INTR,64,16);
		#endif
		#if(( USB_DEV_VENDOR ==1 )||( _DEF_USB_INFO_ == USB_DEV_MSKBD ))
		#if _USB_LS_SPEED
		add_ep_info(hpcd,2,HAL_PCD_EP_TYPE_INTR,8,16);
		#else
		add_ep_info(hpcd,2,HAL_PCD_EP_TYPE_INTR,64,16);
		#endif
		#endif		
#endif
...
#if ( _DEF_USB_INFO_ == USB_UAC_CONSUMER_KBD )
		flush_ep_fifo(hpcd,ISO_EP);
        flush_ep_fifo(hpcd,CONTROL_EP);
		#ifdef ADD_KEYBOARD
		flush_ep_fifo(hpcd,KEYBOARD_EP);
		#endif
#else
		flush_ep_fifo(hpcd,1);
		#if(( USB_DEV_VENDOR ==1 )||( _DEF_USB_INFO_ == USB_DEV_MSKBD ))
		flush_ep_fifo(hpcd,2);	
		#endif
#endif
...
//        dev_sts.d32 = hpcd->dev_glb_regs->DEV_STS;
		...
		dev_sts.d32 = hpcd->dev_glb_regs->DEV_STS;
		flush_ep_fifo(hpcd,ISO_EP);
		extern void set_iso_tranf_control(bool A);
		if(dev_sts.b.ALT==1)
		{
			set_iso_tranf_control(true);
		}
		else
		{
			set_iso_tranf_control(false);
		}
		osal_set_event(application_TaskID, ISOC_TRANSFER_EVT);
		#endif
...
//        LOG_DEBUG("Suspend Int\n");
        ...
        hpcd->dev_glb_regs->DEV_INT_STS = dev_int_clr.d32;
        if(hpcd->dev_glb_regs->DEV_STS & BIT(12))//check SUSP bit
        {
