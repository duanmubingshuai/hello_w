...
/********************************************************************************* 
 * 
 * @brief	Phy6230 soc supports the maximum adc clock selection of 125KHz when tested by adc polling,
 * so 16us must be selected when the acquisition frequency is higher than 125KHz.
 * 
 * @param	uint8_t adc_samptimer[];
 * 
 *********************************************************************************/
//const uint8_t adc_samptime[4] = {1, 2, 40, 80};			//00: 250k,01:125k,10:6250,11:3125
const uint8_t adc_samptime[4] = {1, 4, 40, 80};			//00: 250k,01:125k,10:6250,11:3125

const uint8_t s_pinmap[ADC_CH_NUM] = {GPIO_DUMMY, P7, P4, P5, P6, P9, P10, P11, P12, P13};
...
int set_sampling_resolution(adc_ch_t ch, adc_resol_t resolution)
{
	...
	if(ch == ADC_3V3)
	{
		pcrm_ana_ctl1_vdd3_det_en_setf(1);
	}	
...
uint32_t hal_adc_sample(adc_ch_t ch, adc_resol_t resolution, int samples)
{
	...
	gpio_cfg_analog_io(ch, Bit_ENABLE);
