...
#elif(1)
	#if _USB_LS_SPEED
    gpio_write(USB_BOND_DM, 0);
    gpio_write(USB_BOND_DP, 1);
    WaitMs(10);

    gpio_pull_set(USB_BOND_DM, GPIO_FLOATING);
    gpio_pull_set(USB_BOND_DP, GPIO_FLOATING);
    gpio_dir(USB_BOND_DM, GPIO_INPUT);
    gpio_dir(USB_BOND_DP, GPIO_INPUT);		
	#else    
    gpio_write(USB_BOND_DP, 0);//
    gpio_write(USB_BOND_DM, 1);
    WaitMs(10);
...
//	LOG("int_val = %x\n", (* (volatile int *) 0xe000e100));	//0x4f00810
    ...
    dev_cfg.d32            = hpcd->dev_glb_regs->DEV_CFG;
#if _USB_LS_SPEED
    dev_cfg.b.SPEED        = USB_LS_SPEED_MODE;
#else    
